function h = biplotModif(coefs,varargin)
...
if in3D
    ...
    varHndl = [line(arx(1:2,:),ary(1:2,:),arz(1:2,:), 'Color',[0.9,0.9,0.9], 'LineStyle','-', plotArgs{:}, 'Marker','none','LineWidth',0.001 ); ...
               line(arx(2:3,:),ary(2:3,:),arz(2:3,:), 'Color','b', 'Marker','v','MarkerSize',2,'MarkerFaceColor','b', plotArgs{:}, 'LineStyle','none')];
else
    varHndl = [line(arx(1:2,:),ary(1:2,:), 'Color',[0.9,0.9,0.9], 'LineStyle','-', plotArgs{:}, 'Marker','none','LineWidth',0.001); ...
               line(arx(2:3,:),ary(2:3,:),'Color','b', 'Marker','v','MarkerSize',2,'MarkerFaceColor','b', plotArgs{:}, 'LineStyle','none')];
end
...
if ~isempty(varlabs)
    ...
    delx = .001*diff(get(cax,'XLim'));
    dely = .001*diff(get(cax,'YLim'));
    if in3D
        delz = .001*diff(get(cax,'ZLim'));
    end

    if in3D
        varTxtHndl = text(coefs(:,1)+delx,coefs(:,2)+dely,coefs(:,3)+delz,varlabs,'FontSize',5);
    else
        varTxtHndl = text(coefs(:,1)+delx,coefs(:,2)+dely,varlabs,'FontSize',5);
    end
